...
const PresenzButton = ({ config, callbacks, container, children = 'Verify Identity', className = '', disabled = false, theme = 'brand', }) => {
    ...
    return (jsxs(Fragment, { children: [jsx("button", { type: "button", className: `presenz-button presenz-theme-${theme} ${className}`, onClick: startVerification, disabled: disabled || isLoading, children: isLoading ? (jsx("span", { className: "presenz-loading", children: "Loading..." })) : (children) }), modalOpen && !container && (jsx("div", { className: "presenz-modal-overlay", onClick: handleClose, children: jsxs("div", { className: "presenz-modal-content", onClick: (e) => e.stopPropagation(), children: [jsx("button", { className: "presenz-modal-close", onClick: handleClose, children: "\u00D7" }), errorMessage ? (jsxs("div", { className: "presenz-error", children: [jsx("p", { children: errorMessage }), jsx("button", { onClick: cleanup, children: "Close" })] })) : (jsx("div", { className: "presenz-iframe-wrapper", children: sessionUrlRef.current && (jsx("iframe", { ref: iframeRef, src: sessionUrlRef.current, allow: "camera; microphone; fullscreen; autoplay", sandbox: "allow-scripts allow-same-origin allow-forms allow-popups allow-modals", title: "Presenz Identity Verification" })) }))] }) }))] }));
};
